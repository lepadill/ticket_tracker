...
class generate_tracker:    
    ...
    def get_checker(self,user,bkcChecker):
        ...
        #self.checker_data = self.checker_data.replace(':','":"').replace(' ','')
        #self.checker_data = self.checker_data
        #self.checker_data = self.checker_data[:-1]
        #print('WEEEEEEEEE')
        #print(self.checker_data)
        #print('WEEEEEEEEE')
        #self.checker_data = os.popen('python bkcManager.py -b '+checker+' -s').read()
        #self.checker_data = self.checker_data.replace('\n',',')[:-1].split(',')
        #self.checker_data = self.checker_data.replace(':','":"').replace('\n','","').replace(' ','')
        #self.checker_data = '"'+self.checker_data[:-2]
        #print('AAAA \n')
        #print(self.checker_data)
        self.checker_data = '{'+self.checker_data+'}'
...
def main():
    ...
    #print(BKC_Checker)
        #a = 'hhhuuuh '+str(BKC_Checker['osArbFwVersion'])
        #print(a)
